...
class CourtMasterController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($request->court as $key => $value) {
            ...
            $existingCourt = CourtsMaster::where('district_id', $request->district_id)
                ->where('name', $value['name'])
                ->first();
            if ($existingCourt) {
                return response()->json(['errors' => ['court_name' => 'Court name already exists for this district.']], 422);
            }
