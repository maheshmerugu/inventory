                        ...
                        <form id="company_form" method="POST">
                            @csrf                               
                                <div class="form-group">
                                    ...
                                    <input type="text" id="subject" value="{{$item->subject}}" name="subject" class="form-control" required>
                                </div>
                                <div class="form-group">
                                    <label for="message">Message:</label>
                                    <textarea id="message" name="message" rows="3"  class="form-control" required>{{$item->message ?? ''}}</textarea>
                                </div>
...
<script>
    $("#company_form_btn").click(function(e) {
        ...
        $.ajax({
            url: "{{ route('inventory.request.update', ['id' => $item->id]) }}", // Adjust the route with item ID parameter
            type: "POST",
            data: data,
            dataType: "JSON",
            processData: false,
